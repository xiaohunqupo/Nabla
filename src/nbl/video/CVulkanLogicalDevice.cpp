...
CVulkanLogicalDevice::CVulkanLogicalDevice(core::smart_refctd_ptr<const IAPIConnection>&& api, renderdoc_api_t* const rdoc, const IPhysicalDevice* const physicalDevice, const VkDevice vkdev, const SCreationParams& params) :
    ILogicalDevice(std::move(api),physicalDevice,params,rdoc), m_vkdev(vkdev), m_devf(vkdev),
    m_deferred_op_mempool({.composed={.addrAllocCtorExtraParams={static_cast<uint32_t>(sizeof(CVulkanDeferredOperation))},.blockSizeKBLog2=12}})
{
...
core::smart_refctd_ptr<IGPUImage> CVulkanLogicalDevice::createImage_impl(IGPUImage::SCreationParams&& params, bool dedicatedOnly)
{
    if (!params.valid())
    {
        m_logger.log("Invalid params were given", system::ILogger::ELL_ERROR);
        return nullptr;
    }
...
core::smart_refctd_ptr<IGPUDescriptorSetLayout> CVulkanLogicalDevice::createDescriptorSetLayout_impl(const std::span<const IGPUDescriptorSetLayout::SBinding> bindings, const uint32_t maxSamplersCount)
{
    ...
    for (const auto& binding : bindings)
    {
        ...
        if ((binding.type == asset::IDescriptor::E_TYPE::ET_SAMPLER || binding.type==asset::IDescriptor::E_TYPE::ET_COMBINED_IMAGE_SAMPLER) && binding.immutableSamplers && binding.count)
        {
